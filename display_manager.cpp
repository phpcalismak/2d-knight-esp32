...
void drawHealthPickUp() {
    if (healthPickUp.active) {
        ...
        // Noktayı çiz (basit bir daire veya kare olabilir)
        tft.fillCircle(healthPickUp.x, healthPickUp.y, healthPickUp.size / 2, healthPickUp.color); // Daire çizimi için
        // Ya da tft.fillRect(healthPickUp.x - healthPickUp.size / 2, healthPickUp.y - healthPickUp.size / 2, healthPickUp.size, healthPickUp.size, healthPickUp.color); // Kare çizimi için
...
void drawCharacterSprite(Player& p) {
    ...
    // Bu temizleme, karakterin hareket ettiğinde arkasında "iz" bırakmaması için kritiktir.
    ...
    // Bu pushImage çağrısında transparent parametresi vermek gerekli değildir,
    // çünkü zaten sprite buffer'ının kendisini çiziyoruz ve aşağıda pushSprite ile şeffaflığı yöneteceğiz.
    ...
    // Bu, sprite'taki TRANSPARENT_COLOR piksellerinin ana ekrana çizilmemesini sağlar.
...
void initSprites() {
    ...
    player1Sprite.setSwapBytes(true); // Byte sıralaması doğru olduğundan emin ol

    player2Sprite.setColorDepth(16); // RGB565 için 16-bit
    player2Sprite.createSprite(maxSpriteWidth, maxSpriteHeight);
    player2Sprite.setSwapBytes(true); // Byte sıralaması doğru olduğundan emin ol

    // Arka plan sprite'ını ekranın TAM boyutunda oluştur
    bgSprite.setColorDepth(16); // Arka planın da RGB565 olması ideal
    bgSprite.createSprite(SCREEN_WIDTH, SCREEN_HEIGHT); // EKRAN BOYUTU KADAR! (320x240)
    bgSprite.setSwapBytes(true);

    uiSprite.setColorDepth(8); // UI daha düşük bit derinliği kullanabilir
    uiSprite.createSprite(SCREEN_WIDTH, 20); // UI için üst bant
...
void drawSlashEffect(int x, int y, bool facingRight, int power) {
    ...
    int size = 10 + (power * 5);
    // Bu efekt doğrudan tft ekranına çiziliyor. Eğer arka plan güncelleniyorsa,
    // bu efektin de sonraki karede temizlenmesi gerekebilir.
    if (facingRight) {
